...
# Save bar graph
def bar_graph(event_codes, components):
    # counting components
    component_count = {}
...
def plot_stuff():
    with open(graphs_output_file_path, newline='') as file:
        ...
        lines = list(reader)
        datetimes = ['2020-' + month_to_number(line[1]) + '-' + 
                     ('%02d' % int(line[2])) + 'T' + 
                     line[3] for line in lines[1:]]
        components = [line[5] for line in lines[1:]]
        event_codes = [int(line[8][1:]) for line in lines[1:]]
        levels = [line[4] for line in lines[1:]]
    ...
    plt.figure(figsize=(15, 10))
    bar_graph(event_codes, components)
    plt.close()
